...
_MODE_A_SYSTEM = (
    ...
    "'morph') — describe continuous physical motion only. The video model "
    "can CUT to a different shot mid-clip when a prompt implies a scene "
    "change; that must never happen, so END every motion_prompt with the "
    "sentence 'One continuous shot, no cuts.' Just before it, add one short "
    "stability clause naming what must stay unchanged in that clip (e.g. "
    "'The kitchen, the lighting, and their clothes stay exactly the same.') "
    "— the model drifts on backgrounds, faces, and wardrobe it is not told "
    "to hold. Keep each motion_prompt concrete and compact (one to three "
    "short sentences plus those two closing clauses), in present tense; "
    "preserve each person's identity, wardrobe, and environment except for "
    "the changes visible between the frames; no hard cuts, no people who "
    "appear in neither frame, no on-screen text. Do not "
