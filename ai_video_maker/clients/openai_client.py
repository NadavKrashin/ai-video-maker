...
_MODE_A_SYSTEM = (
    ...
    "is fine there.) "
    "SAME PERSON, CHANGED CLOTHES/AGE/SETTING: when the two frames show the "
    "same person but the outfit, age, or location differs, keep that person "
    "continuously visible and IN MOTION while the change happens — the "
    "clothing and surroundings evolve progressively around the moving "
    "subject, or the change is hidden by a brief natural occlusion (passing "
    "behind furniture, through a doorway, behind a foreground object). Never "
    "park the subject motionless while the world changes around them: an "
    "idle subject invites the video model to crossfade the whole screen, "
    "which looks like a cheap slideshow. "
    "PACING: spread the action across the clip's ENTIRE duration — the "
    "subject starts moving within the first second and keeps moving; do not "
    "hold a static pose for most of the clip and cram the scene change into "
    "the last moment. "
