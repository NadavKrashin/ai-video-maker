...
//
// The panel is served BY that same server (admin_ui/dist mounted at /), so
// every request is same-origin and only the admin token needs entering.

const LS_TOKEN = 'avm_admin_api_token';

export const getToken = () => localStorage.getItem(LS_TOKEN) || '';
export const saveSettings = (token) => {
